...
export default function UserDetailHeader() {
  ...
  if (brokerMetricsAdminQeurry?.isLoading)
    return (
      <div className="h-auto">
        <CenterLoader />
      </div>
    );
  if (brokerMetricsAdminQeurry?.isError)
    return (
      <ShowError
        message={brokerMetricsAdminQeurry?.error?.response?.data?.message}
      />
    );
  console.log("brokerMetricsAdminQeurry", brokerMetricsAdminQeurry?.data);
  return (
    <div className="bg-[#F5F0EC] rounded-lg py-5 px-7 my-4 text-secondary flex justify-between items-center ">
      <div className="flex items-center gap-4">
        <img
          src={
            brokerMetricsAdminQeurry?.data?.signedUrl || "/dummy-profile.png"
          }
          alt="User"
        ...
        <div>
          ...
          <p className="text-tertiary text-sm">
            {brokerMetricsAdminQeurry?.data?.email}
          </p>
          <div className="flex items-center gap-2 text-sm mt-1">
            <span
              className={`${
                brokerMetricsAdminQeurry?.data?.status === "ACTIVE"
                  ? "bg-[#1E8221]"
                  : brokerMetricsAdminQeurry?.data?.status === "DELETED"
                    ? " bg-destructive/80"
                    : "bg-[#A2781E]"
      ...
      <div className="flex items-center justify-between gap-4  flex-[0_0_50%]">
        <div className="flex gap-5 2xl:gap-10 w-full">
          ...
          <div className="w-full xl:whitespace-nowrap">
            <p className="text-[#6B5E55] text-sm 2xl:text-lg">
              Associated Organisation
            </p>
            <p className="font-semibold mt-1 2xl:text-2xl">
              {brokerMetricsAdminQeurry?.data?.relationship
          ...
          <div className="w-full xl:whitespace-nowrap">
            ...
            <p className="font-semibold mt-1 2xl:text-2xl">
              {format(
                brokerMetricsAdminQeurry?.data?.createdAt,
                "MMM dd, yyyy",
              )}
            </p>
          </div>
          ...
          <div className="w-full  xl:whitespace-nowrap">
            ...
            <p className="font-semibold mt-1 2xl:text-2xl">
              {format(
                brokerMetricsAdminQeurry?.data?.lastLogin,
                "MMM dd, yyyy",
              )}
            </p>
          </div>
        ...
        <div className="border border-[#E3D8D2] rounded-md p-4 max-w-[9rem] min-w-[8rem] w-full text-center xl:whitespace-nowrap">
          ...
          <p className="font-semibold text-2xl text-[#2A1A14]">
            {brokerMetricsAdminQeurry?.data?.searchCount}
          </p>
        </div>
