  ...
  const addToDynamoDB = async (address, searchId, userId) => {
    try {
      ...
      if (
        ...
        userType = agentDetail?.data?.getAgent?.__typename?.toLowerCase();
      } else {
        const brokerDetail = await API.graphql(
          graphqlOperation(getBroker, { id: userId }),
        );
        username = brokerDetail?.data?.getBroker?.name;
        userType = brokerDetail?.data?.getBroker?.__typename?.toLowerCase();
      }
    ...
    >
      ...
      <div className="flex gap-3 items-center mb-6 pl-4">
        <Checkbox
          ...
          disabled={loading}
        />
        <Label htmlFor="checkbox" className="mb-0 text-base font-normal">
