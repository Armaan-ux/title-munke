...
import { getBrokerAgentsDetails, getOrganisationAgentDetails, getOrganisationBrokerDetails } from "../service/userAdmin";
...
function OrganisationDetails() {
 ...
 const [activeTab, setActiveTab] = useState(tabTypes[0]);
  ...
  const brokersAgentListingAdminQuery = useQuery({
    queryKey: [queryKeys.brokersAgentListingAdmin,activeTab.id, id, date?.from, date?.to],
    queryFn: () => activeTab.id === "broker" ? getOrganisationBrokerDetails(id, true, date?.from, date?.to) : getOrganisationAgentDetails(id, true, date?.from, date?.to),
    enabled: !!id,
  })
      ...
      <div className="bg-[#F5F0EC] rounded-lg p-7 my-4 text-secondary">
        ...
        <div className="flex justify-between items-center gap-4 mb-6">
          <div className="flex items-center gap-6">
            <p className="text-secondary font-medium text-xl">{activeTab.id === "broker"? "All Broker" : "All Agents" } </p>
          </div>
                    ...
                    <TableRow key={item.id} className="text-black" >
                      ...
                      <TableCell className="text-center" >
                          <Button
                            ...
                            variant="ghost"
                            onClick={() =>
                              navigate( activeTab.id === "broker" ? `/admin/dashboard/broker-property-search/${item?.brokerId}` : `/admin/dashboard/property-search/${item?.agentId}`)
                            }
                          >
                            <Eye />
                          </Button>
