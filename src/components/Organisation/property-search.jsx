...
function PropertySearch() {
  ...
  return (
    <>
      ...
      {agentMetricsAdminQeurry?.isLoading && (
        <div className="h-auto">
          <CenterLoader />
        </div>
      )}
      {agentMetricsAdminQeurry?.isError && (
        <ShowError
          message={agentMetricsAdminQeurry?.error?.response?.data?.message}
        />
      )}
      {agentMetricsAdminQeurry?.isSuccess && (
        <AgentDetailHeader data={agentMetricsAdminQeurry?.data} />
      )}
      <div className="bg-[#F5F0EC] rounded-lg p-7 my-4 text-secondary">
        <div className="bg-white !p-4 rounded-xl">
          <div className="flex justify-between items-center gap-4 mb-6">
            ...
            <DateFilter
              handleFilter={(fromDatetime, toDatetime) =>
                setDate((pre) => ({ ...pre, fromDatetime, toDatetime }))
              }
            />
          </div>
          {agentSearchesQuery?.isLoading && <CenterLoader />}
          {agentSearchesQuery?.isError && (
            <ShowError
              message={agentSearchesQuery?.error?.response?.data?.message}
            />
          )}
                ...
                {agentSearchesQuery?.data?.length === 0 ? (
                  ...
                  agentSearchesQuery?.data?.map((item, index) => (
                    <TableRow key={item.id}>
                      ...
                      <TableCell>
                        {format(item?.createdAt, "MMM d, yyyy, HH:mm:ss")}
                      </TableCell>
                      ...
                      <TableCell>
                        ...
                        >
                          ...
                          <Button
                            ...
                            onClick={() =>
                              navigate(
                                `/broker/dashboard/property-details/${item?.searchId}`,
                              )
                            }
                          >
                            <Eye />
